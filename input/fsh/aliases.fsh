

//============== ALIAS ===============

//General

Alias: $uri = urn:ietf:rfc:3986
Alias: $oid = urn:ietf:rfc:1155


//Patient
Alias: $patient-citizenship = http://hl7.org/fhir/StructureDefinition/patient-citizenship
Alias: $patient-birthPlace = http://hl7.org/fhir/StructureDefinition/patient-birthPlace
//Alias: $istat-luogoNascita = https://www.hl7.it/fhir/terminology/ValueSet/istat-luogoNascita
Alias: $istat-luogoNascita = http://hl7.it/fhir/itcore/ValueSet/vs-istat-luogoNascita
Alias: $patient-nationality = http://hl7.org/fhir/StructureDefinition/patient-nationality
//Alias: $istat-titoloStudio = https://www.hl7.it/fhir/terminology/ValueSet/istat-titoloStudio
Alias: $cs-titoloStudio = http://hl7.it/fhir/itcore/CodeSystem/istat-ctsi03
Alias: $istat-titoloStudio = http://hl7.it/fhir/itcore/ValueSet/vs-istat-titoloStudio
//Alias: $vs-istat-professione = https://www.hl7.it/fhir/terminology/ValueSet/istat-professione
Alias: $vs-istat-professione = http://hl7.it/fhir/itcore/ValueSet/vs-istatprofessioni
Alias: $cs-istatprofessione = http://hl7.it/fhir/itcore/CodeSystem/istat-professioni
//Alias: $istat-cittadinanza = https://www.hl7.it/fhir/terminology/ValueSet/istat-cittadinanza
Alias: $istat-cittadinanza = http://hl7.it/fhir/itcore/ValueSet/vs-istat-cittadinanza
//Alias: $tipoIdentificatore = https://www.hl7.it/fhir/terminology/ValueSet/VstipoIdentificatore
Alias: $cs-tipoIdentificatore = http://terminology.hl7.org/CodeSystem/v2-0203
Alias: $tipoIdentificatore = http://hl7.it/fhir/itcore/ValueSet/vs-tipoIdentificatore
Alias: $iso21090-ADXP-streetName = http://hl7.org/fhir/StructureDefinition/iso21090-ADXP-streetName
Alias: $iso21090-ADXP-streetNameBase = http://hl7.org/fhir/StructureDefinition/iso21090-ADXP-streetNameBase
Alias: $iso21090-ADXP-streetNameType = http://hl7.org/fhir/StructureDefinition/iso21090-ADXP-streetNameType
Alias: $iso21090-ADXP-houseNumber = http://hl7.org/fhir/StructureDefinition/iso21090-ADXP-houseNumber
//Alias: $cs-istat-dug = https://www.hl7.it/fhir/terminology/CodeSystem/dug
Alias: $cs-istat-dug = http://hl7.it/fhir/itcore/CodeSystem/cs-istatdug
//Alias: $vs-istat-dug = https://www.hl7.it/fhir/terminology/ValueSet/istat-dug
Alias: $vs-istat-dug = http://hl7.it/fhir/itcore/ValueSet/vs-istatdug
Alias: $iso21090-code-string = http://hl7.org/fhir/StructureDefinition/iso21090-codedString
Alias: $add-use  = http://hl7.org/fhir/address-use
Alias: $vs-addr-use-org = 	http://hl7.org/fhir/ValueSet/address-use|4.0.1
Alias: $vs-addr-use-it = 	http://hl7.it/fhir/itcore/ValueSet/vs-add-use-it
Alias: $cs-addr-use-org = http://hl7.org/fhir/address-use
//Alias: $vs-stato = https://www.hl7.it/fhir/terminology/ValueSet/istat-stato
Alias: $vs-stato = http://www.hl7.it/fhir/itcore/ValueSet/vs-istatunitaAmministrativeTerritorialiEstere
//Alias: $cs-titoloStudio = https://www.hl7.it/fhir/terminology/CodeSystem/istat-ctsi03
//Alias: $cs-istatprofessione = https://www.hl7.it/fhir/terminology/CodeSystem/istat-professioni
//Alias: $cs-istat-statoNascita = https://www.hl7.it/fhir/terminology/CodeSystem/istat-unitaAmministrativeTerritorialiEstere
Alias: $cs-istat-statoNascita = http://hl7.it/fhir/itcore/CodeSystem/istat-unitaAmministrativeTerritorialiEstere
Alias: $cs-istat-comune = http://hl7.it/fhir/itcore/CodeSystem/istat-unitaAmministrativeTerritoriali
//Alias: $vs-istat-comune = https://www.hl7.it/fhir/terminology/ValueSet/istat-comune
Alias: $vs-istat-comune = http://hl7.it/fhir/itcore/ValueSet/vs-istatcomune

//Practitioner e PractitionerRole
//Alias: $cs-role-it = https://www.hl7.it/fhir/terminology/CodeSystem/it-V3RoleCode
Alias: $cs-role-it = http://hl7.it/fhir/itcore/CodeSystem/it-V3RoleCode
Alias: $vs-role-it = http://hl7.it/fhir/itcore/ValueSet/vs-role-code-it
//Alias: $cs-stato = http://www.hl7.it/fhir/terminology/CodeSystem/istat-unitaAmministrativeTerritorialiEstere

//Alias: $cs-provincia = http://www.hl7.it/fhir/terminology/CodeSystem/province-istat
Alias: $cs-provincia = http://hl7.it/fhir/itcore/CodeSystem/cs-province-istat
//Alias: $cs-comuni = https://www.hl7.it/fhir/terminology/CodeSystem/istat-comuni
//Alias: $cs-regioni = https://www.hl7.it/fhir/terminology/CodeSystem/minsan-regione
//Alias: $vs-regioni = https://www.hl7.it/fhir/terminology/ValueSet/minsan-regione
Alias: $cs-regioni = http://hl7.it/fhir/itcore/CodeSystem/cs-minsan-regione
Alias: $vs-regioni = http://hl7.it/fhir/itcore/ValueSet/vs-minsan-regione
Alias: $cs-speciality = http://snomed.info/sct

//Coverage
Alias: $coverage-exceptionStatus = http://hl7.it/fhir/itcore/StructureDefinition/coverage-exceptionStatus
//Alias: $vs-stato-esenzione = https://www.hl7.it/fhir/terminology/ValueSet/statoEsenzione
//Alias: $cs-statoEsenzione = https://www.hl7.it/fhir/itcore/CodeSystem/cs-stato-esenzione
Alias: $V3ActStatus = http://terminology.hl7.org/CodeSystem/v3-ActStatus
Alias: $v3-NullFlavor = http://terminology.hl7.org/CodeSystem/v3-NullFlavor
//Alias: $cs-esenzione = http://terminology.hl7.org/CodeSystem/v3-ActStatus
Alias: $dataAbsentReasonExt = http://hl7.org/fhir/StructureDefinition/data-absent-reason
//Alias: $cs-codici-esenzioni = https://www.hl7.it/fhir/terminology/CodeSystem/mds-esenzioni
Alias: $vs-esenzione-ne = http://hl7.it/fhir/itcore/ValueSet/vs-esenzione-ne
Alias: $cs-coveragetype = 	http://terminology.hl7.org/CodeSystem/v3-ActCode

//Medication
//Alias: $vs-whoatc = https://www.hl7.it/fhir/terminology/ValueSet/who-atc
Alias: $vs-whoatc = http://hl7.it/fhir/itcore/ValueSet/vs-whoatc
//Alias: $cs-whoatc = http://www.whocc.no/atc
Alias: $cs-whoatc = http://hl7.it/fhir/itcore/CodeSystem/aifa-atc
//Alias: $vs-aifa-ge = https://hl7.it/fhir/itcore/ValueSet/vs-aifa-ge
Alias: $cs-aifa-ge = http://hl7.it/fhir/itcore/CodeSystem/aifa-gruppo-equivalenza
Alias: $vs-nullFlavor = http://terminology.hl7.org/ValueSet/v3-NullFlavor
Alias: $cs-nullFlavor = http://terminology.hl7.org/CodeSystem/v3-NullFlavor
//Alias: $vs-aifa-aic = https://www.hl7.it/fhir/terminology/ValueSet/aifa-aic
Alias: $ext-originalText = http://hl7.org/fhir/StructureDefinition/originalText

//Organization
//Alias: $vs-asl =  https://www.hl7.it/fhir/terminology/ValueSet/uri-idAslRegione
//Alias: $cs-asl = https://www.hl7.it/fhir/terminology/CodeSystem/cs-asl
//Alias: $vs-aslRegione = https://www.hl7.it/fhir/terminology/ValueSet/mds-asl-vs
Alias: $vs-aslRegione = http://hl7.it/fhir/itcore/ValueSet/vs-mds-asl
Alias: $cs-aslRegione = http://hl7.it/fhir/itcore/CodeSystem/cs-mds-asl
//Alias: $vs-aziendaOspedaliera = https://www.hl7.it/fhir/terminology/ValueSet/minsan-idAziendeOspedaliere
// Alias: $cs-aziendaOspedaliera = https://www.hl7.it/fhir/terminology/CodeSystem/minsan-hsp
Alias: $vs-aziendaOspedaliera = http://hl7.it/fhir/itcore/ValueSet/vs-struttura-azienda
Alias: $cs-aziendaOspedaliera = http://hl7.it/fhir/itcore/CodeSystem/minsan-hsp
//Alias: $vs-struttura = https://www.hl7.it/fhir/terminology/ValueSet/minsan-idStrutture
//Alias: $vs-strutturaInterna = https://www.hl7.it/fhir/terminology/ValueSet/minsan-idStruttureInterne 
//Alias: $cs-strutturaInterna = https://www.hl7.it/fhir/terminology/CodeSystem/cs-mds-idStruttureInterne

//Alias: $vs-tipoOrg = https://www.hl7.it/fhir/terminology/ValueSet/tipoOrgSocioSanitaria
Alias: $CS-tipoEntita = http://hl7.it/fhir/itcore/CodeSystem/it-tipoEntita

